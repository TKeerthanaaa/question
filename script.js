...
const questions = [
  ...
  },
  {
    ...
    options: ["Australia", "Indonesia", "Belize", "Brazil"],
    selected: undefined,
    correct: 0,
  },
  {
...
const checkOption = () => {
  for (let i = 0; i < optionsElement.length; i++) {
    optionsElement[i].classList.length === 2
      ? ""
      : optionsElement[i].classList.remove("selected-btn");
  }
};
...
const displaySelectedOptions = (index) => {
  if (questions[index].selected !== undefined) {
    for (let i = 0; i < optionsElement.length; i++) {
      questions[index].selected === optionsElement[i].innerText
        ? optionsElement[i].classList.add("selected-btn")
        : "";
    }
  }
};
...
nextBtnElement.addEventListener("click", () => {
  ...
  if (index < questions.length) {
    ...
    if (
      index > 0 &&
      prevBtnElement.classList.value ===
        "fa-solid fa-circle-chevron-left fa-2xl disabled-btn"
    ) {
